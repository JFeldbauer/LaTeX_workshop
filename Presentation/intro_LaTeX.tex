...
\end{document}
  ...
  \begin{column}{0.2\textwidth}
    \begin{tikzpicture}
    \draw
    [decorate,decoration={brace,mirror,amplitude=10pt},xshift=0pt,yshift=0pt]
    (-3,3.95) -- (-3,4.25) node[black,midway,xshift=1.5cm]{\footnotesize
    Documentclass};\pause
    \draw
    [decorate,decoration={brace,mirror,amplitude=10pt},xshift=0pt,yshift=0pt]
    (-3,2.65) -- (-3,3.85) node[black,midway,xshift=1.5cm]{\footnotesize
    Preamble};\pause
    \draw
    [decorate,decoration={brace,mirror,amplitude=10pt},xshift=0pt,yshift=0pt]
    (-3,-2.2) -- (-3,2.5) node[black,midway,xshift=1.5cm]{\footnotesize
    Document};
\end{tikzpicture}
  \end{column}
\end{columns}
...
\documentclass{beamer}
  % begin preamble
\usepackage{abstract}       % a package that provides an abstract environment
\usepackage{float}          % provide floating environments
\usepackage{amssymb}        % mathematical symbols
\usepackage{hyperref}       % package for cross-references
\usepackage[round]{natbib}  % package for bibliography

\bibliographystyle{style}   % set the style of citations
\title{good read}           % title of the document
\author{Anna und Arthur}    % author name

\beamertemplatenavigationsymbolsempty       % no navigation symbols on pdf
\usecolortheme{beaver}                      % color theme of the pdf
\setbeamertemplate{footline}[frame number]  % frame number in the footline
  % begin text body
\begin{document}
  \end{lstlisting}
\end{frame}
